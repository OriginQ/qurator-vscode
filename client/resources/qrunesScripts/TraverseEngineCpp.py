...
quantum_logic_gate_name = ['H', 'X', 'NOT', 'T', 'Y', 'Z', 'S', 'X1', 'Y1', 'Z1', 'U4', 'RX',
                           'RY', 'RZ', 'CNOT', 'CZ', 'CR', 'CU', 'Toffoli', 'iSWAP', 'Measure', 'MeasureAll',
...
class TraverEngineCpp(QcodeVisitor):
    ...
    def trans_and_build(self, ctx: QcodeParser.Lang_unitContext, st):
        ...
        if isinstance(st[1], VariableSymbol):
            ...
            if t_st[1].type == "circuit":
                self.fn.write('QCircuit '+t_st[1].name)
                self.fn.write(" = CreateEmptyCircuit()")
            if t_st[1].type == "let":
                self.fn.write('auto ' + t_st[1].name)
                self.fn.write(" = ")
    ...
    def visitParameter_specifier(self, ctx: QcodeParser.Parameter_specifierContext):
        if ctx.quantum_primary_type() is not None:
            ...
            return self.visitArray_construct_type(ctx.array_construct_type())
            ...
            return self.visitQuantum_alg_built_in_type(ctx.quantum_alg_built_in_type())
    ...
    def visitQuantum_alg_built_in_type(self, ctx: QcodeParser.Quantum_alg_built_in_typeContext):
        ...
        if ctx.HAMILTONIAN() is not None:
            quantum_alg_built_in_type = 'QHamiltonian'
        elif ctx.AVAR() is not  None:
            quantum_alg_built_in_type = 'var'
        return quantum_alg_built_in_type
    ...
    def visitArray_construct_type(self, ctx: QcodeParser.Array_construct_typeContext):
        ...
        if ctx.VECTOR() is not None:
            array_construct_type += 'vector<'
            array_construct_type += self.visitConstruct_primary_type(ctx.construct_primary_type())+'>'
        else:
            array_construct_type += str(ctx.MAP())
        return array_construct_type
    ...
    def visitConstruct_primary_type(self, ctx: QcodeParser.Construct_primary_typeContext):
        construct_primary_type = ''
        if ctx.auxiliary_primary_type() is not None:
            construct_primary_type+=str(ctx.auxiliary_primary_type().getText())
        elif ctx.quantum_primary_type() is not None:
            construct_primary_type+='Qubit*'
        elif ctx.classical_primary_type() is not None:
            construct_primary_type+='cbit'
        elif ctx.MAP() is not None:
            construct_primary_type+='auto'

        return construct_primary_type
    ...
    def visitDeclarate_function(self, ctx: QcodeParser.Declarate_functionContext):
        ...
        if ctx.function_return_type_declarator() is not None:
            ...
            if func_type == 'circuit<vector<qubit>,qubit>':
                func_type ='Oracle<QVec, Qubit*>'
        else:
            func_type = "qprog"
    ...
    def visitExpression_list(self, ctx: QcodeParser.Expression_listContext, scope):
        ...
        if ctx.exp_list_alias is not None:
            exp_literal = exp_literal + self.visitExpression_list(ctx.exp_list_alias, scope)
        if ctx.COMMA() is not None:
            exp_literal = exp_literal + str(ctx.COMMA()) + " "
        if ctx.single_expression() is not None:
            exp_literal = exp_literal + self.build_exp(ctx.single_expression(), scope)
        return exp_literal

    # Visit a parse tree produced by QcodeParser#vector_expression.
    def visitVector_expression(self, ctx: QcodeParser.Vector_expressionContext, scope,arr_name):
        exp_literal = ""
        if ctx.single_expression() is not None:
            exp_literal = exp_literal + self.build_exp(ctx.single_expression(), scope)
        if ctx.vector_slice() is not None:
            exp_literal = exp_literal + self.visitVector_slice(ctx.vector_slice(), scope,arr_name)
        return exp_literal

    # Visit a parse tree produced by QcodeParser#vector_slice.
    def visitVector_slice(self, ctx: QcodeParser.Vector_sliceContext, scope,arr_name):
        exp_literal = ""
        if ctx.vector_bound_alias_l is not None:
            exp_literal = exp_literal + arr_name +'.begin() + ' +self.build_exp(ctx.vector_bound_alias_l, scope)
        if ctx.COLON() is not None:
            exp_literal = exp_literal + ','
        if ctx.vector_bound_alias_r is not None:
            exp_literal = exp_literal + arr_name +'.begin() + ' + self.build_exp(ctx.vector_bound_alias_r, scope)
        return exp_literal
    ...
    def visitSingle_expression(self, ctx: QcodeParser.Single_expressionContext, scope):
        ...
        elif ctx.function_call_exp is not None:
    ...
    def build_exp(self, ctx: QcodeParser.Single_expressionContext, scope):
        ...
        if ctx.get_index_exp is not None:
            # assign_exp_left = ctx.parentCtx.assign_exp_left
            arr_name = self.build_exp(ctx.get_index_exp, scope)
            if self.is_assign_exp_left == 1:
                exp_literal = exp_literal + \
                              self.visitVector_expression(ctx.vector_expression(), scope,arr_name)
                self.is_assign_exp_left = 0
            else:
                exp_literal = exp_literal + arr_name+  str(ctx.LBRACK()) \
                              + self.visitVector_expression(ctx.vector_expression(), scope,arr_name) + str(ctx.RBRACK())
        ...
        if ctx.function_call_exp is not None:
            if ctx.function_call_exp.getText()[-7:] == '.length':
                exp_literal = exp_literal+ctx.function_call_exp.getText()[:-7]+'.size'+ \
                              str(ctx.LPAREN())
                ...
                exp_literal = exp_literal+ctx.function_call_exp.getText()[:-7]+'.push_back'+ \
                              str(ctx.LPAREN())
                if ctx.expression_list() is not None:
                    exp_literal = exp_literal + self.visitExpression_list(ctx.expression_list(), scope) + ")"
                else:
                ...
                if ctx.expression_list() is not  None:
                    ...
                    exp_literal = exp_literal + new_str+ ')'
                else:
                    exp_literal = exp_literal + ')'
            elif ctx.function_call_exp.getText()[-7:] == '.dagger':
                exp_literal = exp_literal + self.build_exp(ctx.function_call_exp, scope)+'()'
            elif ctx.function_call_exp.getText()[-9:] == '.getFirst':
                exp_literal = exp_literal + ctx.function_call_exp.getText()[:-9]+'.first'
            elif ctx.function_call_exp.getText()[-10:] == '.getSecond':
                exp_literal = exp_literal + ctx.function_call_exp.getText()[:-10]+'.second'
            elif ctx.function_call_exp.getText()[-6:] == '.first':
                exp_literal = exp_literal + ctx.function_call_exp.getText()[:-6]+'.first'
            elif ctx.function_call_exp.getText()[-7:] == '.second':
                exp_literal = exp_literal + ctx.function_call_exp.getText()[:-7]+'.second'
            elif ctx.function_call_exp.getText()[-7:] == '.remove' :
                exp_literal = exp_literal + ctx.function_call_exp.getText()[:-7]+'.erase'+ \
                              str(ctx.LPAREN())
                if ctx.expression_list() is not  None:
                    new_str = ctx.expression_list().getText()
                    str_arr = new_str.split(',')
                    exp_literal = exp_literal + ctx.function_call_exp.get_inside_exp.getText() + '.begin()+' + str_arr[0] + ', '+ \
                                        ctx.function_call_exp.get_inside_exp.getText()+ '.begin()+' + str_arr[0] + '+1)'
            elif ctx.function_call_exp.getText()[-4:] == '.pop' :
                exp_literal = exp_literal + ctx.function_call_exp.getText()[:-4]+'.erase'+ \
                              str(ctx.LPAREN())
                if ctx.expression_list() is not  None:
                    new_str = ctx.expression_list().getText()
                    str_arr = new_str.split(',')
        ...
        if ctx.assign_op() is not None:
            if ctx.assign_exp_right.get_index_exp is not None and \
                    scope.symbols[ctx.assign_exp_left.getText()].type[:6] == 'vector':
                self.is_assign_exp_left = 1
                exp_literal = exp_literal + ctx.assign_exp_left.getText() + '.insert(' +ctx.assign_exp_left.getText()\
                              +'.begin(),'+\
    ...
    def visitLambda_exp(self, ctx: QcodeParser.Lambda_expContext, scope):
        ...
        string_of_paras = self.fn.trans_list_2_string_cpp(lambda_paras)
    ...
    def visitLambda_body(self, temp_info):
        ...
        self.fn.write("QCircuit circuit;")
        ...
        self.fn.createline()
        self.fn.createIdent(function_scope.scope_level - 2)
        self.fn.write("};")
    ...
    def visitLogic_op(self, ctx: QcodeParser.Logic_opContext):
        ...
        if ctx.LOGIC_OR() is not None:
            exp_literal = " " + str(ctx.LOGIC_OR()) + " "
        if ctx.LOGIC_AND() is not None:
            exp_literal = " " + str(ctx.LOGIC_AND()) + " "
        return exp_literal
    ...
    def visitStatement(self, ctx: QcodeParser.StatementContext, scope, func_return):
        ...
        if ctx.while_statement() is not None:
            # self.fn.createline()
            # self.fn.createIdent(scope.scope_level - 1)
    ...
    def visitReturn_statement(self, ctx: QcodeParser.Return_statementContext, scope, func_return):
        return_content = ""
        if ctx.single_expression().lambda_exp() is not None:
            func_paras_str = ''
            for parameters in scope.symbols:
                func_paras_str+=','+parameters
            func_paras_str = func_paras_str.strip(string.punctuation)
            return_content += ctx.RETURN().getText() + " ["+func_paras_str+"]" + self.build_exp(ctx.single_expression(), scope)
    ...
    def visitQif_statement(self, ctx: QcodeParser.Qif_statementContext, scope, func_return, qif_flag):
        ...
        self.fn.createline()
        self.fn.createIdent(qif_scope.enclosing_scope.scope_level - qif_flag)
        self.fn.write( 'QIfProg ' + qif_prog_name)
    ...
    def visitExpression_statement(self, ctx: QcodeParser.Expression_statementContext, scope, func_return):
        ...
        if func_return == "qprog":
            if exp_type == "circuit" or exp_type == "qprog" or exp_type == "logic_gate" or exp_type == "measure_op"\
                    or exp_type == 'circuit<vector<qubit>,qubit>':
                self.fn.write(func_return)
                self.fn.write(" << ")
                self.fn.write(exp_literal)
            else:
    ...
    def visitDeclaration_variable_statement(self, ctx: QcodeParser.Declaration_variable_statementContext, scope, func_return):
        ...
        if var_symbol.type == "circuit":
            ...
            if var_symbol.data is not None and var_symbol.data != '':
                ...
                self.fn.createline()
                self.fn.createIdent(scope.scope_level - 1)
                declara_content = func_return + " << " + var_symbol.name
        ...
        if var_symbol.type == "vector<qubit>":
            ...
            if var_symbol.data != '':
                ...
                self.fn.createIdent(scope.scope_level - 1)
                self.fn.write(var_symbol.name)
                self.fn.write(".insert("+var_symbol.name+'.begin(),')
                self.fn.write(var_symbol.data)
                self.fn.write(")")
                self.fn.write(';')
                self.is_assign_exp_left = 0
    ...
    def visitFor_stmt(self, ctx: QcodeParser.For_stmtContext, scope):
        ...
        for_header = "for(" +  for_start_info[0]+ ' ' + for_start_info[1]+ '  = ' + for_start_info[2]+';'+ \
                     for_start_info[1] + ' < ' +for_condi + ';'+for_start_info[1] + ' += ' + for_step+'){'
    ...
    def visitInhanced_for_stmt(self, ctx: QcodeParser.Inhanced_for_stmtContext, scope):
        construct_primary_type = self.visitConstruct_primary_type(ctx.construct_primary_type())
        iterator = ctx.Identifier().getText()
        ite_obj = self.build_exp(ctx.single_expression(), scope)
        inhanced_for_header = "for" + "( "+construct_primary_type +" "+ iterator + " : " + ite_obj + "){"
    ...
    def build_function_scope(self, ctx: QcodeParser.Function_definitionContext, func_symbol):
        ...
        for para_item in func_symbol.params:
            function_scope.insert(para_item)
            cpp_paras_type = para_item.type
            if cpp_paras_type == 'qprog':
                cpp_paras_type = 'QProg'
            elif cpp_paras_type == "circuit":
                cpp_paras_type = 'QCircuit'
            elif cpp_paras_type == "cbit":
                cpp_paras_type = 'ClassicalCondition'
            elif cpp_paras_type == "vector<qubit>":
                cpp_paras_type = 'QVec'
            elif cpp_paras_type == "vector<cbit>":
                cpp_paras_type = 'vector<ClassicalCondition>'
            elif cpp_paras_type == "circuit<vector<qubit>,qubit>":
                cpp_paras_type = 'Oracle<QVec, Qubit*>'
            based_cpp_paras.append(cpp_paras_type + ' ' + para_item.name)
        ...
        cpp_func_type = func_return
        if func_return == "qprog":
            cpp_func_type = 'QProg'
        elif func_return == "circuit":
            cpp_func_type = 'QCircuit'
        elif func_return == "variationalCircuit":
            cpp_func_type = 'VQC'
        elif func_return == "cbit":
            cpp_func_type = 'ClassicalCondition'
        elif func_return == "vector<qubit>":
            cpp_func_type = 'QVec'
        elif func_return == "vector<cbit>":
            cpp_func_type = 'vector<ClassicalCondition>'
        elif func_return == "circuit<vector<qubit>,qubit>":
            cpp_func_type = 'Oracle<QVec, Qubit*>'


        func_header = cpp_func_type + ' ' + func_symbol.name + "(" + string_of_paras + "){"
        ...
        if func_return == "qprog":
            self.fn.createline()
            self.fn.createIdent(function_scope.scope_level - 1)
            self.fn.write("QProg qprog = CreateEmptyQProg();")
            ...
            self.fn.createline()
            self.fn.createIdent(function_scope.scope_level - 1)
            self.fn.write("QCircuit circuit = CreateEmptyCircuit();")
            self.visitFunction_body(ctx.function_body(), function_scope, func_return)
            self.fn.createline()
            self.fn.createIdent(function_scope.scope_level - 1)
            self.fn.write("return ")
            ...
            self.fn.createline()
            self.fn.createIdent(function_scope.scope_level - 1)
            self.fn.write("VQC variationalCircuit = VariationalQuantumCircuit();")
            self.visitFunction_body(ctx.function_body(), function_scope, func_return)
            self.fn.createline()
            self.fn.createIdent(function_scope.scope_level - 1)
            self.fn.write("return ")
